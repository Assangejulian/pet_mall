...
async function main() {
  ...
  // Optional: test that text setting works
  const testSlide = p.slides.items[14];
  const testShape = testSlide.shapes.items[5]; // 矩形5: "系统设计"
  const before = String(testShape.text);
  testShape.text = "系统设计 测试";
  const after = String(testShape.text);
