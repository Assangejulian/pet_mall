<template>
  <div class="admin-page">
    <div class="page-header"><h2>我的门店</h2><p>维护自己的门店资料与审核状�?/p></div>
    <div class="search-bar">
      <input v-model="keyword" placeholder="搜索门店名称" @keyup.enter="search" />
      <select v-model.number="statusFilter"><option :value="-1">全部状�?/option><option :value="0">待审�?/option><option :value="1">营业�?/option><option :value="2">已关�?/option><option :value="3">审核驳回</option></select>
    ...
    <div class="table-wrap">
      <table class="data-table"><thead><tr><th>门店</th><th>联系电话</th><th>地址</th><th>商品�?/th><th>状�?/th><th>审核/关闭说明</th><th>操作</th></tr></thead>
        <tbody>
          ...
          <tr v-if="!loading && !records.length"><td colspan="7" class="empty-row">暂无门店</td></tr><tr v-if="loading"><td colspan="7" class="empty-row">加载�?..</td></tr>
        </tbody>
      </table>
      <div class="pagination" v-if="total"><button :disabled="page <= 1" @click="go(page-1)">上一�?/button><span>�?{{ page }} / {{ totalPages }} 页，�?{{ total }} �?/span><button :disabled="page >= totalPages" @click="go(page+1)">下一�?/button></div>
    </div>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="showModal=false"><div class="modal-card">
      ...
      <div class="modal-body">
        ...
        <p class="hint">门店资料提交后将重新进入待审核状�?/p>
      </div>
      <div class="modal-footer"><button class="btn btn-outline" @click="showModal=false">取消</button><button class="btn btn-primary" :disabled="saving" @click="save">{{ saving ? '保存�?..' : '保存' }}</button></div>
    </div></div>
  </div>
</template>
