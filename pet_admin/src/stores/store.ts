...
export const useStoreStore = defineStore("admin_store", () => {
  ...
  async function fetch(params: { current: number; size: number; keyword?: string; status?: number; city?: string }) { loading.value = true; const r = await listStores(params); list.value = r.records; total.value = r.total; loading.value = false }
