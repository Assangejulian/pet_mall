...
export const useProductStore = defineStore("admin_product", () => {
  ...
  async function fetch(params: { current: number; size: number; keyword?: string; type?: string; category?: string; storeId?: string; status?: string }) { loading.value = true; const r = await listProducts(params); list.value = r.records; total.value = r.total; loading.value = false }
