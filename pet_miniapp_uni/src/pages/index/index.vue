<template>
  <view class="page">
    <view class="header">
      <text class="eyebrow">瑁佺獫瑾栭爤</text>
      <text class="title">鐧肩従姝ｅ湪鐧肩敓鐨勫鐗╃灛闁?</text>
      <text class="subtitle">鐎戝竷娴佹祻瑙堬紝榛炲暦鍚岄€插叆鍏ㄥ睆鐭珫鏈戦€茶銆?/text>
    </view>

    <view class="mock-tip" v-if="useMock">鐣惰█浣跨敤鏈湴婕旂ず鏁告摎</view>
    ...
    <view class="masonry" v-else>
      <view class="column">
        ...
        >
          ...
          <view class="play-mark">鈻?/view>
          <view class="tile-copy">
            <text class="video-title">{{item.title}}</text>
            <text class="video-desc">{{item.desc}}</text>
            <view class="meta">
              <text class="author">{{item.author}}</text>
              <text class="count">{{item.likes}} 璧?/text>
            </view>
          </view>
          <view class="product-link" v-if="item.productId" @tap.stop="goProduct(item.productId)">闁滆仈鍟嗗搧</view>
        </view>
      </view>
...
const fallbackVideos = [
  ...
  },
  {
    ...
    url: "http://video.sonetto.online/Golden_retriever_holding_tulip_g鈥_202607022323.mp4",
    author: "",
    likes: 980,
    playCount: 5000,
    productId: 1
  ...
  },
  {
    ...
    title: "",
    desc: "",
    cover: "/static/images/mock/blue-cat.jpg",
...
.cover-shade {
  ...
  top: 0;
  bottom: 0;
  background: linear-gradient(180deg, rgba(0, 0, 0, 0.03) 24%, rgba(0, 0, 0, 0.72) 100%);
}
