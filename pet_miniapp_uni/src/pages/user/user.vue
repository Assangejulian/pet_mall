  ...
  <view class="page">
    ...
    <view v-else>
      <view class="profile">
        ...
        <view class="pf-inner">
          <view class="pf-avatar">{{(userName||"?")[0]}}</view>
          <view class="pf-info">
            <text class="pf-name serif">{{userName||"用户"}}</text>
            <text class="pf-sub">暖窝会员</text>
          </view>
        </view>
      </view>
      <view class="card">
      ...
      uni.showModal({
        ...
        success: (r) => {
          if (r.confirm) {
            ...
            this.isLogin = false; this.userName = "";
...
.pf-avatar{width:100rpx;height:100rpx;border-radius:50%;background:rgba(232,146,124,.25);color:#e8927c;font-size:36rpx;font-weight:800;display:flex;align-items:center;justify-content:center}
.pf-info{flex:1}.pf-name{display:block;font-family:"Noto Serif SC","Songti SC",serif;font-size:34rpx;font-weight:700;color:#fff}
...
.og{display:flex}.oi{flex:1;display:flex;flex-direction:column;align-items:center;gap:10rpx;padding:8rpx 0}
.oi-icon{font-size:32rpx;color:#8f7366}.oi-lb{font-size:22rpx;color:#8f7366;font-weight:600}
...
.ar{color:#d5c8be;font-weight:700}.logout-btn{justify-content:center;color:#c0392b}
